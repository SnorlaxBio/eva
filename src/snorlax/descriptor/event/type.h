/**
 * @file        snorlax/descriptor/event/type.h
 * @brief
 * @details
 * 
 * @author      snorlax <opuntia@snorlax.bio>
 * @since       June 25, 2024
 */

#ifndef   __SNORLAX__DESCRIPTOR_EVENT_TYPE__H__
#define   __SNORLAX__DESCRIPTOR_EVENT_TYPE__H__

#define descriptor_event_type_subscription          0
#define descriptor_event_type_open                  1
#define descriptor_event_type_read                  2
#define descriptor_event_type_write                 3
#define descriptor_event_type_close                 4
#define descriptor_event_type_exception             5
#define descriptor_event_type_max                   6

#endif // __SNORLAX__DESCRIPTOR_EVENT_TYPE__H__
